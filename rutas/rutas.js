...
router.post('/crear-conexion', async (req, res) => {
  ...
  try {
    ...
    // Si la conexión es exitosa, redirigir a la página de bases de datos
    res.redirect(`/bases-de-datos?host=${host}&username=${username}&password=${password}`);
  } catch (error) {
    console.error('Error al crear la conexión', error);
    res.status(500).send('Error al crear la conexión');
  }
});

  router.get('/crear-conexion', (req, res) => {
    const { host, username, password } = req.query;
    res.render("conexion", { host, username, password });
  });
...
router.get('/bases-de-datos', async (req, res) => {
  ...
  try {
    // Crear una conexión usando los datos proporcionados
    const connection = await mysql.createConnection({
      host: host,
      user: username,
      password: password
